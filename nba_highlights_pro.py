...
def get_player_highlights(game_id, player_id, player_name, is_israeli, stats_line):
    """מוריד קטעים, מחבר אותם ושולח לטלגרם"""
    ...
    try:
        ...
        # סגירת קבצים לשחרור זיכרון RAM (חשוב ל-Railway!)
        final_video.close()
...
def run_highlights_hunter():
    """לולאה ראשית שסורקת משחקים"""
    print("🚀 צייד ההיילייטס התחיל לעבוד! מחפש ישראלים ומשחקי 40+ נק'...")
    ...
    while True:
        try:
            ...
            for g in resp['scoreboard']['games']:
                ...
                # בדיקה אם המשחק הסתיים (Status 3) ושלא טופל
                if g['gameStatus'] == 3 and gid not in PROCESSED_GAMES:
                    print(f"✅ משחק {gid} הסתיים. בודק סטטיסטיקות...")
                    ...
                    for p in all_players:
                        ...
                        if is_israeli or s['points'] >= 30 or s['assists'] >= 20:
                            ...
                            print(f"🌟 נמצא שחקן מתאים: {p_display_name} ({stats_text})")
                            ...
                            if result:
                                ...
                                with open(vid_path, 'rb') as video_file:
                                    requests.post(url, data={
                                        ...
                                        'parse_mode': 'HTML'
                                    }, files={'video': video_file}, timeout=30)
                                
                                if os.path.exists(vid_path): os.remove(vid_path)
                                print(f"🚀 וידאו נשלח בהצלחה עבור {p_display_name}!")
