import requests
import time
import json
from datetime import datetime, timedelta

# ==========================================
# הגדרות מערכת (טוקנים ו-API)
# ==========================================
TELEGRAM_TOKEN = "8514837332:AAFZmYxXJS43Dpz2x-1rM_Glpske3OxTJrE"
CHAT_ID = "-1003808107418"
NBA_URL = "https://cdn.nba.com/static/json/liveData/scoreboard/todaysScoreboard_00.json"
HEADERS = {"User-Agent": "Mozilla/5.0"}

# --- כאן אתה מגדיר את שעות השליחה בקלות ---
SCHEDULE_TIME = "15:15" 
RESULTS_TIME = "15:16"
# ------------------------------------------

def translate_name(name):
    # כאן נכנסת פונקציית התרגום שלך
    return name 

def format_nba_time(time_str):
    """המרה מזמן NBA (ET) לשעון ישראל"""
    try:
        dt = datetime.strptime(time_str, "%Y-%m-%dT%H:%M:%SZ")
        dt_israel = dt + timedelta(hours=7) 
        return dt_israel.strftime("%H:%M")
    except:
        return "TBD"

# ==========================================
# פונקציות עיצוב ההודעות
# ==========================================

def get_schedule_msg(games):
    msg = "‏🏀 <b>לוח משחקי הלילה</b> 🏀\n\n"
    found = False
    for g in games:
        if g['gameStatus'] == 1:
            start_time = format_nba_time(g['gameEt'])
            home = translate_name(f"{g['homeTeam']['teamCity']} {g['homeTeam']['teamName']}")
            away = translate_name(f"{g['awayTeam']['teamCity']} {g['awayTeam']['teamName']}")
            msg += f"‏⏰ <b>{start_time}</b>\n‏🏀 {home} 🆚 {away}\n\n"
            found = True
    return msg if found else "‏🏀 <b>אין משחקים מתוכננים להלילה.</b>"

def get_results_msg(games):
    msg = "‏🏀 <b>תוצאות משחקי הלילה</b> 🏀\n\n"
    found = False
    for g in games:
        if g['gameStatus'] == 3:
            h_name = translate_name(f"{g['homeTeam']['teamCity']} {g['homeTeam']['teamName']}")
            a_name = translate_name(f"{g['awayTeam']['teamCity']} {g['awayTeam']['teamName']}")
            h_score = g['homeTeam']['score']
            a_score = g['awayTeam']['score']
            if h_score > a_score:
                msg += f"‏🏆 <b>{h_name} {h_score}</b>\n‏🔹 {a_name} {a_score}\n\n"
            else:
                msg += f"‏🏆 <b>{a_name} {a_score}</b>\n‏🔹 {h_name} {h_score}\n\n"
            found = True
    return msg if found else "‏🏀 <b>טרם הסתיימו משחקים הלילה.</b>"

def send_to_telegram(text):
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    payload = {"chat_id": CHAT_ID, "text": text, "parse_mode": "HTML"}
    try:
        requests.post(url, data=payload, timeout=15)
    except Exception as e:
        print(f"Error sending to Telegram: {e}")

# ==========================================
# לוגיקת שליחה
# ==========================================

def run_scheduler():
    # פקודת ה-print תוקנה כדי למנוע את שגיאת ה-SyntaxError
    print(f"🚀 הבוט הופעל! לו\"ז: {SCHEDULE_TIME} | תוצאות: {RESULTS_TIME}")
    
    sent_schedule = False
    sent_results = False

    while True:
        now = datetime.now().strftime("%H:%M")
        
        if now == "00:00":
            sent_schedule = False
            sent_results = False

        # בדיקה לשליחת לו"ז
        if now == SCHEDULE_TIME and not sent_schedule:
            try:
                resp = requests.get(NBA_URL, headers=HEADERS).json()
                games = resp.get('scoreboard', {}).get('games', [])
                send_to_telegram(get_schedule_msg(games))
                print(f"✅ לו\"ז נשלח ב-{now}")
                sent_schedule = True
            except Exception as e:
                print(f"❌ שגיאה בלו\"ז: {e}")

        # בדיקה לשליחת תוצאות
        if now == RESULTS_TIME and not sent_results:
            try:
                resp = requests.get(NBA_URL, headers=HEADERS).json()
                games = resp.get('scoreboard', {}).get('games', [])
                send_to_telegram(get_results_msg(games))
                print(f"✅ תוצאות נשלחו ב-{now}")
                sent_results = True
            except Exception as e:
                print(f"❌ שגיאה בתוצאות: {e}")

        time.sleep(20) # בדיקה כל 20 שניות לדיוק מקסימלי

if __name__ == "__main__":
    run_scheduler()
