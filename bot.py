...
def format_msg(box, label, is_final=False):
    ...
    if "יצא לדרך" in label and period == 1:
        ...
        msg += f"{rtl}📍 {b(a_name)}\n"
        msg += f"{rtl}🏀 {b('חמישייה:')} {', '.join(lineups['away']['starters']) if lineups['away']['starters'] else 'טרם פורסם'}\n"
        if lineups['away']['out']:
            msg += f"{rtl}❌ {b('חיסורים:')} {', '.join(lineups['away']['out'][:5])}\n"
        msg += "\n"
        
        msg += f"{rtl}📍 {b(h_name)}\n"
        msg += f"{rtl}🏀 {b('חמישייה:')} {', '.join(lineups['home']['starters']) if lineups['home']['starters'] else 'טרם פורסם'}\n"
        if lineups['home']['out']:
            msg += f"{rtl}❌ {b('חיסורים:')} {', '.join(lineups['home']['out'][:5])}\n"
